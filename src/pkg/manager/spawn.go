...
func (m *Manager) spawnWithRetry(name, command, workdir string) (int, string, error) {
	...
	for attempt := 0; attempt < SpawnRetryAttempts; attempt++ {
		pid, logPath, err := m.spawnOnce(name, wrapped, workdir)
		if err != nil {
		...
		if !logHasTransientExecError(logPath) {
...
// the caller's liveness check. It returns the child pid and its log path.
func (m *Manager) spawnOnce(name, wrapped, workdir string) (int, string, error) {
	logPath := m.newLogPath(name)
...
// logHasTransientExecError reports whether a child's log contains a marker of an
// asynchronous transient execve failure.
func logHasTransientExecError(logPath string) bool {
