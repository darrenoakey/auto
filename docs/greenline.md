...
canonical checkout of `auto`; it is always on `main`, always clean,
and always identical to what prod (`auto`) runs. Every change reaches
...
```
greenline worktree feature-x        # new worktree at /Volumes/Gumby/worktrees/greenline/auto/feature-x
cd /Volumes/Gumby/worktrees/greenline/auto/feature-x         # branch gl/feature-x off last-green
# ...edit, test, commit...
...
- **`./run deploy`** — cwd = the canonical checkout. Rebuild/restart prod (e.g.
  `auto -q restart auto`), **health-check**, exit nonzero on unhealthy, and be
